    ...
    for (var i = 0; i < links.length; i++) {
        ...
        if (p) {
            ...
            for (var k = 0; k < lines.length; k++) {
                ...
                if (line.indexOf('\u6765\u81ea') === 0) device = line;
            }
            var userLinks = p.querySelectorAll('a[href*="/user/"]');
            if (userLinks.length > 0) {
                author = (userLinks[0].textContent || '').trim();
            }
        }
        results.push({
            review_id: rid,
