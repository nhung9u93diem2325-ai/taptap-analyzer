...
def search_game(game_name):
    """
    优先使用 TapTap JSON API 搜索（不依赖 JS 渲染）；
    若 API 无结果，则 fallback 到 HTML 页面解析。
    """
    # ── 方法1：TapTap 搜索 API（推荐，返回 JSON）──
    api_urls = [
        f"https://www.taptap.cn/api/v2/app/list-by-keyword?kw={requests.utils.quote(game_name)}&limit=10",
        f"https://www.taptap.cn/api/v2/search?kw={requests.utils.quote(game_name)}&type=app&limit=10",
    ]
    api_headers = {
        **HEADERS,
    ...
    for api_url in api_urls:
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                # 遍历常见 JSON 结构取 app_id
                items = (
                    data.get("data", {}).get("list")
                    or data.get("data", {}).get("apps")
                    or data.get("list")
                    or []
                )
                if items:
                    ...
                    app_id = (
                        first.get("id")
                        or first.get("app_id")
                        or first.get("app", {}).get("id")
                    )
                    if app_id:
                        print(f"[API] 找到游戏 ID: {app_id}")
                        return str(app_id)
        except Exception as e:
            print(f"[API] 请求失败 {api_url}: {e}")

    # ── 方法2：Fallback —— 抓取搜索结果页（HTML），同时兼容 SSR 和 JSON 内嵌 ──
    fallback_url = f"https://www.taptap.cn/search?q={requests.utils.quote(game_name)}&type=app"
    try:
        resp = requests.get(fallback_url, headers=HEADERS, timeout=20)
        html = resp.text

        # 2a. 页面内嵌 __INITIAL_STATE__ JSON（部分页面 SSR 会带）
            ...
            try:
                ...
                def find_id(obj, depth=0):
                    ...
                    if isinstance(obj, dict):
                        for k in ("id", "app_id"):
                            if k in obj and str(obj[k]).isdigit() and int(obj[k]) > 1000:
                                return str(obj[k])
                        for v in obj.values():
                            r = find_id(v, depth + 1)
                            if r:
                ...
                found = find_id(state)
                if found:
                    print(f"[SSR JSON] 找到游戏 ID: {found}")
                    return found
            except Exception:
                pass

        # 2b. 解析 HTML 链接 /app/数字
        soup = BeautifulSoup(html, "html.parser")
        for a in soup.find_all("a", href=True):
            m2 = re.search(r"/app/(\d+)", a["href"])
            if m2:
                print(f"[HTML] 找到游戏 ID: {m2.group(1)}")
                return m2.group(1)

    except Exception as e:
        print(f"[Fallback HTML] 搜索出错: {e}")
