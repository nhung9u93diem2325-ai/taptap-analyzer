...
def init_driver():
    ...
    # 优先使用环境变量指定的路径（Render/Linux 部署时设置）
    ...
    if driver_path and os.path.exists(driver_path):
        ...
        try:
            ...
            # 最后兜底：直接调用系统 PATH 里的 chromedriver
...
# 搜索游戏（Selenium）
# ────────────────────────────────────────
def search_game_selenium(driver, game_name):
    """用 Selenium 打开搜索页，从渲染后的 DOM 里找 app_id"""
    import urllib.parse
    ...
    for link in links:
        ...
        if m:
            ...
            try:
                ...
                pass
            print(f"[Search] found app_id={app_id} title={title}")
...
# ────────────────────────────────────────
# 爬取评论（Selenium，直接复用你的成熟逻辑）
# ────────────────────────────────────────
def extract_reviews_from_dom(driver):
    """从当前页面 DOM 提取所有评论，与你原脚本逻辑一致"""
    from selenium.webdriver.common.by import By

    reviews = {}
    elements = driver.find_elements(By.CSS_SELECTOR, "a[href*='/review/']")
    ...
    for el in elements:
        ...
        try:
            ...
            """
            container_text = driver.execute_script(js, el)
            ...
            user_els = driver.execute_script("""
                var el = arguments[0];
                var p = el;
                for(var i=0;i<8;i++){
                    p = p.parentElement;
...
def scrape_reviews_selenium(driver, app_id, review_type, task_id, label_key, max_no_change=5, scroll_pause=3):
    """
    滚动爬取评论页，review_type: "差评" 或 "好评"
    URL 参数与你原脚本完全一致
    """
    import urllib.parse
    ...
    while True:
        ...
        current = extract_reviews_from_dom(driver)
...
def run_task(task_id, game_name, app_id_override=None):
    ...
    try:
        ...
        if app_id_override:
            ...
            app_id, resolved_name = search_game_selenium(driver, game_name)
            if not app_id:
                raise Exception(
                    f"未找到游戏《{game_name}》。"
        ...
        bad_reviews = scrape_reviews_selenium(driver, app_id, "差评", task_id, "bad")

        # Step 3: 好评
        tasks[task_id].update({"step": 3, "message": "开始爬取好评..."})
        good_reviews = scrape_reviews_selenium(driver, app_id, "好评", task_id, "good")
