...
def extract_reviews_from_page(page):
    ...
    try:
        js_lines = [
            ...
            "      p = p.parentElement;",
            "      var plen = (p.textContent || '').length;",
            "      if (p.tagName === 'DIV' && plen > 20) break;",
            "    }",
            "    if (p) {",
            "      var allText = p.innerText || p.textContent || '';",
            ...
            "        var c2 = line.charCodeAt(2);",
            "        var c3 = line.charCodeAt(3);",
            ...
            "    }",
            "    results.push({review_id: rid, author: author, content: content, time: pub_time, device: device, likes: '', url: 'https://www.taptap.cn/review/' + rid});",
            "  }",
            "  return results;",
            "})()",
        ]
