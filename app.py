...
def extract_reviews_from_page(page):
    """从当前页面提取所有评论（用 JS 一次性提取，避免 ElementHandle 超时）"""
    reviews = {}
    try:
        # 用单次 JS 调用提取所有数据，避免多次 ElementHandle 操作导致超时
        items = page.evaluate("""() => {
            var results = [];
            var seen = new Set();
            var links = document.querySelectorAll('a[href*="/review/"]');
            links.forEach(function(el) {
                var href = el.getAttribute('href') || '';
                var m = href.match(/\/review\/(\d+)$/);
                if (!m) return;
                var rid = m[1];
                ...
                var author = '', pub_time = '', device = '', likes = '';
                try {
                    var p = el;
                    for (var i = 0; i < 8; i++) {
                        p = p.parentElement;
                    ...
                    if (p) {
                        ...
                        lines.forEach(function(line) {
                            if (/\d{4}\/\d{1,2}\/\d{1,2}/.test(line)) pub_time = line;
                            if (line.startsWith('来自')) device = line;
                            if (/^\d+$/.test(line) && !likes) likes = line;
                        });
                        var userLink = p.querySelector('a[href*="/user/"]');
        ...
        print(f"[extract] JS evaluation error: {e}")
    return reviews
